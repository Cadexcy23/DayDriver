...
float Artist::SCREEN_SCALE = 10;
bool Artist::FULLSCREEN = true;
bool Artist::drawMenus = true;
bool Artist::displayFPS = false;

//TEMP
float Artist::carXPos = 90, Artist::carYPos = 0;
int roadYPos = 0;
bool colorLastGreen = true;
float carR = 0, carG = 0, carB = 0;
Artist::RGB colorA = { 0, 0, 0 };
Artist::RGB colorB = { 0, 0, 0 };
bool initCarColor = false;
int Artist::speed = 1;
std::vector<Artist::pos> barrelList;
int lastBarrelDrop = 1000;
int barrelDropSpeed = 1000;
bool Artist::inControl = false;
bool alive = true;
int distTraveled = 0;
float carExplosionFrame = 0;
...
//images
std::vector<SDL_Texture*> Artist::gCar;
std::vector<SDL_Texture*> Artist::gExplode;
std::vector<SDL_Texture*> Artist::gFire;
std::vector<SDL_Texture*> Artist::gSpeedometer;
SDL_Texture* Artist::gBG = NULL;
SDL_Texture* Artist::gBarrel = NULL;
SDL_Texture* Artist::gCarWrecked = NULL;
...
bool Artist::loadMedia()
{
	...
	Artist::gCar = loadAnimationData("animated", "car", 2);
	Artist::gExplode = loadAnimationData("animated", "explode", 16);
	Artist::gFire = loadAnimationData("animated", "fire", 6);
	Artist::gSpeedometer = loadAnimationData("animated", "speedometer", 9);
	Artist::gBG = loadTexture("Resource/bg.png");
	Artist::gBarrel = loadTexture("Resource/barrel.png");
	Artist::gCarWrecked = loadTexture("Resource/carWrecked.png");
...
void Artist::draw()
{
	...
	//TEMP
	//draw BG
	artist.drawImage(gBG, 0, roadYPos - 108);
	artist.drawImage(gBG, 0, roadYPos);
	...
	{
		artist.drawImage(gBarrel, barrelList[i].x, barrelList[i].y);
	}
	//temp add barrel 
	if (speed > 0)
	{
		if (clock() > lastBarrelDrop + barrelDropSpeed / speed)
		{
			lastBarrelDrop = clock();
			barrelList.push_back({ rand() % 180, -16 });
			inControl = true;
		}
		for (int i = 0; i < barrelList.size(); i++)
		{
			if (Controller::FPSLock)
			{
				barrelList[i].y += speed;
			}
			//delete barrels past under screen
			if (barrelList[i].y > 110)
			{
				barrelList.erase(barrelList.begin());
			}
			//check collision 
			bool colideX = false, colideY = false;
			for (int x = 0; x < 12; x++)
			{
				if (carXPos + x >= barrelList[i].x && carXPos + x <= barrelList[i].x + 12)
				{
					colideX = true;
				}
			}
			for (int y = 0; y < 16; y++)
			{
				if (carYPos + y >= barrelList[i].y && carYPos + y <= barrelList[i].y + 16)
				{
					colideY = true;
				}
			}
	...
	if (alive)
	{
		if (speed == 0)
		{
			artist.drawImage(gCar[0], carXPos, carYPos);
		}
		else
		{
			artist.drawAnimation(gCar, carXPos, carYPos, 60);
		}
		if (Controller::FPSLock)
		{
			//slide car up or down to match speed
			if (speed == 1 && carYPos < 92)
			{
				carYPos++;
			}
			if (speed == 2 && carYPos < 76)
			{
				carYPos++;
			}
			if (speed == 2 && carYPos > 76)
			...
			if (speed == 3 && carYPos > 60)
			{
				carYPos--;
			}
			//change color back and forth
			//set color only once
			if (!initCarColor)
			{
				//define the 2 color points
				//colorA = {  };
				//colorB = {  };
				colorA = { rand() % 255, rand() % 255, rand() % 255 };
				colorB = { rand() % 255, rand() % 255, rand() % 255 };
			...
			//decide how much to change of each
			float carRChange, carGChange, carBChange;
			if (colorLastGreen)
			{
				carRChange = float(colorB.r - colorA.r) / 60;
				carGChange = float(colorB.g - colorA.g) / 60;
				carBChange = float(colorB.b - colorA.b) / 60;
			}
			else
			{
				carRChange = float(colorA.r - colorB.r) / 60;
				carGChange = float(colorA.g - colorB.g) / 60;
				carBChange = float(colorA.b - colorB.b) / 60;
			}
			//add the number to storage
			carR += carRChange;
			carG += carGChange;
			carB += carBChange;
			//check if we hit either color and flip it if so
			if (colorB.r < carR + 1 && colorB.r > carR - 1 && colorB.g < carG + 1 && colorB.g > carG - 1 && colorB.g < carG + 1 && colorB.g > carG - 1)
			{
				colorLastGreen = false;
			}
			if (colorA.r < carR + 1 && colorA.r > carR - 1 && colorA.g < carG + 1 && colorA.g > carG - 1 && colorA.g < carG + 1 && colorA.g > carG - 1)
			{
				colorLastGreen = true;
			}
			//push new color
			SDL_SetTextureColorMod(gCar[0], carR, carG, carB);
			SDL_SetTextureColorMod(gCar[1], carR, carG, carB);
			SDL_SetTextureColorMod(gCarWrecked, carR, carG, carB);
		}
	}
	else // game over stuff
	{
		if (carExplosionFrame < 8)
		{
			artist.drawImage(Artist::gCar[0], carXPos, carYPos);
		}
		else
		{
			artist.drawImage(Artist::gCarWrecked, carXPos, carYPos);
			artist.drawAnimation(Artist::gFire, carXPos, carYPos, 128);
		}
		if (carExplosionFrame < Artist::gExplode.size())
			artist.drawImage(Artist::gExplode[carExplosionFrame], carXPos, carYPos);
		if (Controller::FPSLock)
			carExplosionFrame += .25;

		artist.drawLetters("Distance", 16, 17, Artist::smallFont);
		artist.drawLetters(std::to_string(distTraveled) + "M", 16, 38, Artist::smallFont);
	}
	

	//draw speedomiter
	//determin what frame to draw
	static int drawFrame = 0;
	if (speed != 0)
	{
		if (Controller::FPSLock)
		{
			drawFrame = speed + (speed - 1) * 2;
			drawFrame += (rand() % 3) - 1;
		}
	}
	else
	...
	artist.drawImage(gSpeedometer[drawFrame], 1, 1);

	//draw dist traveled
	//artist.drawLetters("Dist:" + std::to_string(distTraveled) + "Feet", 0, 17, Artist::smallFont);
