...
void Controller::controller()
{
	...
	//TEMP
	if (keyboardStates[SDL_SCANCODE_R] == 1)
	{
		Artist artist;
		artist.resetGame();
	}
	if (Artist::inControl)
	{
		if (keyboardStates[SDL_SCANCODE_W] == 1)
		{
			Artist::speed++;
			if (Artist::speed > 3)
				Artist::speed = 3;
		...
		if (keyboardStates[SDL_SCANCODE_S] == 1)
		{
			Artist::speed--;
			if (Artist::speed < 1)
				Artist::speed = 1;
			//Artist::carYPos += .025;
			//if (Artist::carYPos > 92)
			//{
			//	Artist::carYPos = 92;
			//}
		}
		if (keyboardStates[SDL_SCANCODE_D] == 2)
		{
			Artist::carXPos += .025;
			if (Artist::carXPos > 180)
			{
				Artist::carXPos = 180;
			}
		}
		if (keyboardStates[SDL_SCANCODE_A] == 2)
		{
			Artist::carXPos -= .025;
			if (Artist::carXPos < 0)
			{
